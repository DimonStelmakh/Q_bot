...
async def help(message: types.Message):
    text = '⚙ Всі команди бота <b>Q Bot KPI</b>:\n' \
           ...
           '\n/readme — отримати інформацію про бота та роботу з ним' \
...
async def technical_report(message: types.Message):
    ...
    report = '⚙ Технічний звіт:\n\n' \
             ...
             '🤖 Версія бота: <b>1.0.1</b>\n' \
             '🖥 Розробники:\n' \
             '<i>Дмитро Стельмах (@d_stelmakh7592)\n' \
             'Віталій Солоничний (@Sulphur14)\n' \
             'Вікторія Ріпка (@t0n_am0ur4ik)</i>\n\n' \
             'ℹ Бот створено студентами НТУУ "Київський політехнічний інститут ім. Ігоря Сікорського" у 2023 році ' \
...
async def readme(message: types.Message):
    with open(file='readme.txt', encoding='utf-8', mode='r') as file:
...
async def add_subject(message: types.Message, state: FSMContext):
    ...
    except mysql.connector.IntegrityError as ex_:
        await state.finish()
        await message.answer('😉 Цей предмет уже доданий'
                             '\n\nДодати інший предмет: /add_subject\nДодати викладача: /add_teacher')
        return
    ...
    if not check_database(message):
        try:
            ...
            await message.answer(f"🫡 Розпочинаю роботу в групі {message.chat.title}"
                                 f"\n\n📄 Інформація про бота та як працювати з ним: /readme")
